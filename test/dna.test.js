...
test('DNA.equals compares every gene', () => {
  const {DNA} = loadDNA();
  const dna = new DNA(10, 20, 30, 1);

  assert.equal(dna.equals(new DNA(10, 20, 30, 1)), true);
  assert.equal(dna.equals(new DNA(11, 20, 30, 1)), false);
  assert.equal(dna.equals(new DNA(10, 21, 30, 1)), false);
  assert.equal(dna.equals(new DNA(10, 20, 31, 1)), false);
  assert.equal(dna.equals(new DNA(10, 20, 30, 1.1)), false);
});

test('combine averages genes when mutation does not happen', () => {
  const context = loadDNA();
  const {DNA, combine} = context;
  const child = combine(new DNA(10, 20, 30, 1), new DNA(20, 40, 50, 2));
...
test('default DNA and mutations use the same gene bounds', () => {
  const calls = [];
  const values = [10, 1, 50, 0.5, 0, 12, 0, 34, 0, 56, 0, 0.75];
  ...
  assert.deepEqual(
      {size: child.size, move: child.move, dist: child.dist, foodAttraction: child.foodAttraction},
      {size: 12, move: 34, dist: 56, foodAttraction: 0.75},
  );
});
